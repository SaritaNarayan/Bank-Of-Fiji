...
namespace BoF.Web.Database_Access_Layer
{
    public class db
    {
        ...
        {
            SqlCommand com = new SqlCommand("select FirstName,AccountNumber, AccountName, Balance from Account inner join Customer on Customer.Id = Account.Customer_id inner join AccountType on AccountType.Id = Account.AccountType_id where Customer.Id = @prefix", connection);
        ...
        {
            SqlCommand com = new SqlCommand("select Acc.AccountNumber, AccType.AccountName as AccountType, Trans.TransDateTime as TransactionDate, Trans.TransDetails as Details,Trans.TransAmount as Amount,  Acc.Balance  from Account Acc inner join[BOF].[dbo].[Transaction] Trans on Trans.Account_id = Acc.Id inner join AccountType AccType on AccType.Id = Acc.AccountType_id inner join Customer Customer on Customer.Id = Acc.Customer_id where Customer.Id = = @prefix", connection);
            com.Parameters.AddWithValue("@prefix", prefix);
            DataSet ds = new DataSet();
            SqlDataAdapter da = new SqlDataAdapter(com);
            da.Fill(0, 5000, ds.Tables.Add("MyTable"));
